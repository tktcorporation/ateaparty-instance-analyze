...
interface VRChatWorld {
  ...
  updated_at: string;
  visits: number;
  unityPackages?: Array<Record<string, unknown>>;
  udonProducts?: Array<Record<string, unknown>>;
  urlList?: string[];
  version?: number;
}

interface VRChatInstance {
  instanceId: string;
  location: string;
  memberCount: number;
  world: VRChatWorld;
...
interface InstanceHistory {
  [timestamp: string]: VRChatInstance[];
}

// モックレスポンスデータ
const mockInstanceData: VRChatInstance[] = [
  {
    instanceId:
      '23067~group(grp_e7e50ff7-a7b4-4645-8640-9292957b52fb)~groupAccessType(plus)~region(jp)',
    location:
      'wrld_8f7d823e-79a9-4b8f-9efa-c53deeb1d36c:23067~group(grp_e7e50ff7-a7b4-4645-8640-9292957b52fb)~groupAccessType(plus)~region(jp)',
    memberCount: 17,
    world: {
      ...
      imageUrl:
        'https://api.vrchat.cloud/api/1/file/file_c4ed712b-2d33-45d2-bd0d-e65eab81e3fb/4/file',
      ...
      tags: ['author_tag_agog', 'author_tag_xr', 'author_tag_impact', 'system_approved'],
      thumbnailImageUrl:
        'https://api.vrchat.cloud/api/1/image/file_c4ed712b-2d33-45d2-bd0d-e65eab81e3fb/4/256',
 ...
 */
function saveInstanceHistory(instances: VRChatInstance[], timestamp: string): boolean {
  try {
    ...
    if (fs.existsSync(historyFilePath)) {
      ...
      try {
        history = JSON.parse(historyData) as InstanceHistory;

        // 既に同じキーのデータが存在し、内容も同じ場合は更新しない
        if (
          history[timestamp] &&
          JSON.stringify(history[timestamp]) === JSON.stringify(instances)
        ) {
        ...
        console.warn('履歴ファイルの解析に失敗しました。新しいファイルを作成します。', error);
      }
    }
    ...
    if (fileExisted) {
      ...
      console.log(
        `インスタンス履歴を ${historyFilePath} に新規作成しました。日時キー: ${timestamp}`
      );
    }
 ...
 */
function saveDailyHistory(instances: VRChatInstance[], groupId: string): boolean {
  try {
    ...
    if (fs.existsSync(dailyFilePath)) {
      ...
      try {
        const existingInstances = JSON.parse(existingData) as VRChatInstance[];
        // 内容が同じ場合は更新しない
        ...
        console.warn('日次ファイルの解析に失敗しました。新しいファイルを作成します。', error);
      }
    }
...
async function fetchGroupInstances(groupId: string): Promise<boolean> {
  try {
    ...
    if (!VRC_AUTH_TOKEN) {
      console.warn(
        '警告: 認証情報が設定されていません。多くのAPIエンドポイントでは認証が必要です。'
      );
      console.warn(
        'MOCK_RESPONSE=true を設定してモックデータを使用するか、有効な認証トークンを設定してください。'
      );
    ...
    if (process.env.MOCK_RESPONSE === 'true') {
      ...
      // APIリクエスト送信
      instances = await ofetch<VRChatInstance[]>(url, {
        method: 'GET',
        headers,
        timeout,
        retry: 1,
    ...
    if (error instanceof FetchError) {
      ...
      if (error.status === 401) {
        console.error(
          '認証エラー: VRChatの認証トークンが無効または期限切れです。新しいトークンを取得して設定してください。'
        );
        console.error(
          'VRChatにログインして、ブラウザの開発者ツールでCookieから新しいauthトークンを取得してください。'
        );
      }
    }
  ...
  try {
    ...
    if (fileChanged) {
      ...
      if (process.env.GITHUB_ACTIONS === 'true') {
        ...
        console.log(
          'ファイル変更を検出: GitHub Actionsの環境変数 FILE_CHANGED=true を設定しました'
        );
      }
    } else {
